...
contract RiDiDaAuction is IERC1155Receiver, IERC721Receiver, ReentrancyGuard, Ownable {
	...
	mapping(address => Royalty) public royaltyDetails;
	...
	event RoyaltyDistributed(uint256 auctionId, uint256 percentage, address wallet);
	...
	function addRoyalty(address collection, uint256 percentage, address creatorWallet) external {
		require(msg.sender == CollectionContract(collection).owner() || msg.sender == owner(),"Access Denied");
		require(percentage >= 0 && percentage <= 1000,"Royalty can be 0 to 10");
		royaltyAdded[collection] = true;
		royaltyDetails[collection] = Royalty(
			percentage, creatorWallet
		);
	}
	...
	function transferFunds(uint256 auctionId) private isValidId(auctionId) returns(uint256 buyerPaid) {
		...
		if (idToAuction[auctionId].sellerCurrency == idToAuction[auctionId].bidderCurrency) {
			...
			if (idToAuction[auctionId].sellerCurrency == tokenAddress) {
				uint256 royaltyDistributed;
				if(royaltyAdded[nftContract]){
                  IERC20(tokenAddress).transferFrom(idToAuction[auctionId].bidder, 
				  royaltyDetails[nftContract].royaltyAddress,
				  (idToAuction[auctionId].amount*royaltyDetails[nftContract].royaltyPercent) / 10000);  
				  royaltyDistributed = (idToAuction[auctionId].amount*royaltyDetails[nftContract].royaltyPercent) / 10000;
				  emit RoyaltyDistributed(auctionId, royaltyDetails[nftContract].royaltyPercent, royaltyDetails[nftContract].royaltyAddress);
				}
				IERC20(tokenAddress).transferFrom
				(idToAuction[auctionId].bidder, _seller, (idToAuction[auctionId].amount - royaltyDistributed));
				...
				uint256 royaltyDistributed;
				if(royaltyAdded[nftContract]){
				  payable(royaltyDetails[nftContract].royaltyAddress).
				  transfer((idToAuction[auctionId].amount*royaltyDetails[nftContract].royaltyPercent) / 10000);
				  royaltyDistributed = (idToAuction[auctionId].amount*royaltyDetails[nftContract].royaltyPercent) / 10000;
				  emit RoyaltyDistributed(auctionId, royaltyDetails[nftContract].royaltyPercent, royaltyDetails[nftContract].royaltyAddress);
				}
				payable(_seller).transfer(idToAuction[auctionId].amount  - (sellerFee + royaltyDistributed));
			...
			if (idToAuction[auctionId].bidderCurrency == tokenAddress) {
				...
				uint256 royaltyDistributed;
				if(royaltyAdded[nftContract]){
				  payable(royaltyDetails[nftContract].royaltyAddress).transfer((ethReturned*royaltyDetails[nftContract].royaltyPercent) / 10000);
				  royaltyDistributed = (ethReturned*royaltyDetails[nftContract].royaltyPercent) / 10000;
				  emit RoyaltyDistributed(auctionId, royaltyDetails[nftContract].royaltyPercent, royaltyDetails[nftContract].royaltyAddress);
				}
				payable(_seller).transfer(ethReturned - (sellerFee + royaltyDistributed));
				buyerPaid = tokens;
				...
				uint256 royaltyDistributed;
				if(royaltyAdded[nftContract]){
                  IERC20(tokenAddress).transfer(royaltyDetails[nftContract].royaltyAddress, 
				  (tokensReturned*royaltyDetails[nftContract].royaltyPercent) / 10000);  
				  royaltyDistributed = (tokensReturned*royaltyDetails[nftContract].royaltyPercent) / 10000;
				  emit RoyaltyDistributed(auctionId, royaltyDetails[nftContract].royaltyPercent, royaltyDetails[nftContract].royaltyAddress);
				}
				IERC20(tokenAddress).transfer(_seller, tokensReturned-royaltyDistributed);
				buyerPaid = paid;
			}
